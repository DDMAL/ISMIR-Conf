...
const sel_lbds = d3.select('#sel_lbds');
...
function brush_ended() {
    ...
    sel_lbds.selectAll('.sel_lbd').data(all_sel)
      .join('div')
      .attr('class', 'sel_lbd')
      .html(
        d => `<div class="p_title">${d.content.title}</div> <div class="p_authors">${d.content.authors.join(
          ', ')}</div>`)
      .on('click',
        d => window.open(`lbd_${d.content.iclr_id}.html`, '_blank'))
      .on('mouseenter', d => {
...
const updateVis = () => {
    ...
    l_main.selectAll('.dot').data(all_lbds, d => d.id)
      .join('circle')
      .attr('class', 'dot')
      ...
      .on('click',
        function(d) {
            window.open(`lbd_${d.id}.html`, '_blank');
...
const render = () => {
    ...
    all_lbds.forEach(lbd => lbd.is_selected = test(lbd));
...
const tooltip_template = (d) => `
    <div>
        ...
        <img style="max-width: 80%;" src="static/lbd/ISMIR2020-LBD-${d.id}-thumbnail.png" width=100%/>
        </div>
     </div>
`
...
const start = () => {
    Promise.all([
        d3.json('lbds.json'),
        d3.json('serve_lbds_projection.json')
    ]).then(([lbds, proj]) => {
        // all_proj = proj;
